...
def main
  ...
  OptionParser.new do |opts|
    opts.on('-s', '--short', 'Short output') { short = true }
  end.parse!

  abort 'Usage: git-list-changes [-s] <commit-range> [dir]' if ARGV.empty?

  from, to = parse_range(ARGV[0])
  dir = ARGV[1] || ''
  modules = dir.empty? ? find_modules : [dir]

  modules.each { |mod| process_module(mod, from, to, short) }
end

def parse_range(range)
  parts = range.split('..')
  parts.size == 2 ? parts : [range, 'origin/master']
end
...
def process_module(mod, from, to, short)
  git_log = `git log origin/master --oneline #{from}..#{to} --grep "^Merged" -- #{mod}`.strip
